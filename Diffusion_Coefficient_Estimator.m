function diff = Diffusion_Coefficient_Estimator(AVAR_small,AVAR_large,t_small,t_large,diff_drift)
...
diff_white=sqrt(AVAR_small*sqrt(t_small));

% For (relatively) large time t_large, flicker noise dominates.

diff_flicker=sqrt(3*AVAR_large)/sqrt(t_large*sqrt(t_large));
